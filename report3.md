...
Finally, to more examine the groups at a more granular level, we broke down each group from the Food Pyramid and laid out their distributions among the 6 K-Means food groups in Figure 6 above. The percentages shown display how much of each food group from the Food Pyramid is present in each food group from K-Means, so the 78% in the top right corner means that 78% of the foods from the Meats food group as defined by the Food Pyramid is present in the High Protein group as defined by K-Means. Small percentages close to 0 can be attributed to error from modeling. Looking across the table, it is clear that while a few of the groups, such as Meats and Fats, are pretty well defined into a single group and thus match up quite well with the Food Pyramid's categorization, most of the groups are distributed into multiple groups, with Dairy splitting into as many as four different K-Means groups. Fruits splits into potentially three, with its smallest distribution of 9% in the High Carb, High Sugar group, the food group that is technically supposed to describe the Fruits group. The scattered distributions in this table very clearly break down the categorizations of food from the Food Pyramid.
